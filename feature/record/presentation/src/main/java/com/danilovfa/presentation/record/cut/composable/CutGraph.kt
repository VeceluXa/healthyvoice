package com.danilovfa.presentation.record.cut.composable
...
import com.danilovfa.presentation.record.cut.store.CutStoreExecutor.Companion.MIN_CUT_SIZE_MILLIS
...
internal fun CutGraph(
    ...
    val minCutPx = screenWidthPx / durationMillis * MIN_CUT_SIZE_MILLIS

    val amplitudeWidthPx = (screenWidthPx / amplitudes.size) * 2 / 3
    val distanceBetweenAmplitudesPx = (screenWidthPx / amplitudes.size) / 3
    ...
    var startOffsetPx by remember {
        mutableFloatStateOf(screenWidthPx / durationMillis * startOffsetMillis)
    }

    var endOffsetPx by remember {
        mutableFloatStateOf(screenWidthPx / durationMillis * endOffsetMillis)
    }

    LaunchedEffect(startOffsetPx) {
        val newStartOffsetMillis = (startOffsetPx / screenWidthPx * durationMillis).roundToInt()
        onStartOffsetChanged(newStartOffsetMillis)
    }

    LaunchedEffect(endOffsetPx) {
        val newEndOffsetMillis = (endOffsetPx / screenWidthPx * durationMillis).roundToInt()
            ...
            .pointerInput(Unit) {
                detectHorizontalDragGestures { change, dragAmount ->
                    ...
                    if (distanceToStart < distanceToEnd) {
                        ...
                        if (newEndOffsetPx - startOffsetPx > minCutPx && newEndOffsetPx <= screenWidthPx) {
