...
import com.danilovfa.presentation.record.cut.CutComponent.Output
...
import com.danilovfa.presentation.record.cut.store.CutStoreFactory

internal class DefaultCutComponent(
    ...
    private val store = instanceKeeper.getStore {
        CutStoreFactory(storeFactory).create(recordingId = recordingId)
    }
    ...
    init {
        ...
        observeLabels(store.labels) { label ->
            when (label) {
                is Label.NavigateAnalyze -> output(Output.Analyze(recordingId))
