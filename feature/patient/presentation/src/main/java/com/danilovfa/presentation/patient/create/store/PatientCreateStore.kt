...
internal interface PatientCreateStore : Store<Intent, State, Label> {
    ...
    data class State(
        ...
        private fun isDataChanged(): Boolean =
            patient != null && (patient.name != name || patient.birthDate != birthDate ||
                    patient.sex != sex || patient.note != note)

        private fun areAllFieldsFilled(): Boolean = name.isNotEmpty() && birthDate != null
    }
